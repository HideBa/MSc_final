...
% \todo{Ravi's comment "FYI this has mostly to do with the OGC requirement that states that geometry should be fetchable in WGS84 (aside from the Dutch CRS), so not really relevant here I think. Instead you can point out that 1) it requires persistence server process and 2) does not support attribute queries aside from ID lookup"}
...
\subsubsection{Variable-Length Value Serialisation}
...
The index is structured to optimize for \ac{http} Range Requests, with several techniques employed to minimize network overhead:
