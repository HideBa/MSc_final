...
% The spatial index in FlatCityBuf implements a packed R-tree structure with Hilbert curve ordering, heavily inspired by and adapted from the FlatGeoBuf format \citep{Worace2022}. This approach offers several advantages over traditional R-tree variants:
...
% \label{subsec:spatial_index_performance}

% The packed Hilbert R-tree implementation demonstrates several important performance characteristics:

% \begin{itemize}
%     \item \textbf{Query Complexity}: $O(\log_B n)$ node accesses for typical queries, where $B$ is the branching factor
%     \item \textbf{Storage Efficiency}: Approximately 40 bytes per node, with total index size proportional to the number of features
%     \item \textbf{HTTP Efficiency}: Typically requires 3-5 HTTP requests to satisfy a bounding box query on moderate-sized datasets
%     \item \textbf{Build Time}: $O(n \log n)$ for sorting plus $O(n)$ for tree construction
% \end{itemize}

% Benchmark testing with various datasets showed that the spatial indexing mechanism reduces data transfer by up to 95\% for typical bounding box queries compared to downloading the entire dataset, with query times improved by similar factors.

% \subsection{Limitations and Future Directions}
% \label{subsec:spatial_index_limitations}

% The current spatial indexing implementation has several limitations that could be addressed in future work:

% \begin{itemize}
%     \item \textbf{2D Limitation}: The index does not support true 3D volumetric queries, which could be valuable for some specialised applications
%     \item \textbf{Static Structure}: The immutable nature of the index requires rebuilding the entire file to incorporate new features
%     \item \textbf{Single Index Type}: Only R-tree indexing is currently supported, though other spatial index structures might offer advantages for specific use cases
% \end{itemize}

% Future directions could include optional 3D indexing for applications requiring volumetric queries, exploration of compressed index representations, and investigation of hybrid indexing approaches that combine spatial and attribute-based filtering.
